...
const fetchAndStoreProducts = async (): Promise<void> => {
  try {
    ...
    const products = response.data.items.map((item: any) => {
      ...
      const category_ids = item.extension_attributes?.category_links?.map((cat: any) => cat.category_id) || [];

      const customAttributes = Object.fromEntries(
        (item.custom_attributes || []).map((attr: any) => [attr.attribute_code, attr.value])
      );
      ...
      return {
        ...
        category_ids,
        cost: customAttributes.cost ? parseFloat(customAttributes.cost) : undefined,
        special_price: customAttributes.special_price ? parseFloat(customAttributes.special_price) : undefined,
